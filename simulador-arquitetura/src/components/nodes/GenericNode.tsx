      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <div className={`relative bg-white rounded-md shadow-md ${selected ? 'ring-2 ring-blue-500' : ''}`}>
            ...
            {/* Conteúdo do nó */}
            <div className={`flex flex-col items-center p-2 border-2 border-gray-400 rounded-md`}>
              <div className="text-2xl mb-1">
                <Cpu className="h-6 w-6 text-gray-600" />
              </div>
              <div className="text-xs font-medium text-center max-w-[80px] truncate">{data.label || 'Componente Genérico'}</div>
              <div className="text-[10px] text-gray-500 mt-1">
                {data.serviceType || 'Service'} • {data.latency || 10}ms
              </div>
            </div>
          </div>
        </TooltipTrigger>
        <TooltipContent side="bottom">
          <div className="space-y-1 p-1">
            <p className="font-medium text-sm">{data.label || 'Componente Genérico'}</p>
            <p className="text-xs">Tipo: {data.serviceType || 'Service'}</p>
