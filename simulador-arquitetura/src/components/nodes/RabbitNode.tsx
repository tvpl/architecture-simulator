...
export function RabbitNode({ data, selected }: NodeProps) {
  return (
      ...
      <Tooltip>
          ...
          <div className={`relative bg-white rounded-md shadow-md ${selected ? 'ring-2 ring-blue-500' : ''}`}>
            ...
            <div className={`flex flex-col items-center p-2 border-2 border-pink-400 rounded-md`}>
              <div className="text-2xl mb-1">
                <Rabbit className="h-6 w-6 text-pink-500" />
              </div>
              <div className="text-xs font-medium text-center max-w-[80px] truncate">{data.label || 'RabbitMQ'}</div>
        ...
        <TooltipContent side="bottom">
          <div className="space-y-1 p-1">
            <p className="font-medium text-sm">{data.label || 'RabbitMQ'}</p>
            <p className="text-xs">Filas: {data.queues || 1}</p>
            <p className="text-xs">Exchanges: {data.exchanges || 1}</p>
