...
import { X, AlertTriangle, Info, AlertCircle, Loader2, TrendingUp, DollarSign, Zap, Network } from "lucide-react";
...
export function SimulationPanel() {
  ...
  return (
    <div className="absolute bottom-0 left-0 right-0 h-80 z-20 bg-background border-t border-border flex flex-col shadow-xl">
      {/* Header */}
            ...
            {result.resourceUtilization.map((res) => (
              <div key={res.nodeId} className="rounded-lg border border-border p-3">
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <span className={cn(
                    "text-xs font-bold",
                    res.utilization >= 90 ? "text-red-600 dark:text-red-400" :
                    res.utilization >= 70 ? "text-yellow-600 dark:text-yellow-400" :
                ...
                <Progress
                  ...
                  indicatorClassName={cn(
                    res.utilization >= 90 ? "bg-red-500" :
                    res.utilization >= 70 ? "bg-yellow-500" :
                    "bg-green-500"
                  )}
                />
...
function RecommendationCard({ rec }: { rec: Recommendation }) {
  const Icon = rec.severity === "critical" ? AlertCircle :
               rec.severity === "warning" ? AlertTriangle : Info;
  const badgeVariant = rec.severity === "critical" ? "critical" :
    ...
    <div className="rounded-lg border border-border bg-muted/20 p-2.5 space-y-1">
      <div className="flex items-start gap-2">
        <Icon className={cn(
          ...
          rec.severity === "warning" ? "text-yellow-500" : "text-blue-500"
        )} />
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5 flex-wrap">
            <span className="text-xs font-medium">{rec.title}</span>
            <Badge variant={badgeVariant} className="text-[9px]">{rec.category}</Badge>
          </div>
