...
export function FlowCanvas() {
  ...
  const startSimulation = useCallback(async () => {
    ...
    try {
      ...
      alert('Ocorreu um erro durante a simulação. Verifique se a API está rodando e tente novamente.');
    } finally {
  ...
  // Função para mostrar o painel de status da API
  const showApiStatusPanel = useCallback(() => {
    panelManager.showPanel('apiStatus', {
      title: 'Status da API',
      content: (
        <ApiStatus onClose={() => panelManager.hidePanel('apiStatus')} />
      ),
      position: { x: 20, y: window.innerHeight - 400 },
      width: 350,
      height: 350
    });
  }, [panelManager]);
      ...
      <div className="w-full h-full">
        ...
        >
          ...
          <Panel position="top-left" className="flex gap-2">
            ...
            <Button
              onClick={showApiStatusPanel}
              className="h-8 text-xs bg-blue-200 text-blue-800"
              variant="outline"
            >
              Status API
            </Button>
